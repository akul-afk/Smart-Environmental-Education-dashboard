...
class HomeWidget(QWidget):
    ...
    def _build_ui(self, stats, next_badges, streak_label, progress):
        ...
        if stats:
            ...
            xp_frame.setStyleSheet(f"""
                QFrame {{ border-image: url('assets/textures/cards/home_hero_card.png') 0 0 0 0 stretch stretch; border-radius: 15px; }}
            """)
    ...
    def _progress_card(self, icon, label, pct, color, detail):
        frame = QFrame()
        frame.setStyleSheet(f"QFrame {{ border-image: url('assets/textures/cards/home_progress_card.png') 0 0 0 0 stretch stretch; border-radius: 12px; }}")
    ...
    def _badge_preview(self, nb):
        ...
        frame.setStyleSheet(f"""
            QFrame {{ border-image: url('assets/textures/cards/home_badge_card.png') 0 0 0 0 stretch stretch; border-radius: 10px; }}
        """)
        col = QVBoxLayout(frame)
    ...
    def _feature_card(self, emoji, title, desc, color, nav_idx):
        ...
        frame.setStyleSheet(f"""
            QFrame {{ border-image: url('assets/textures/cards/home_quick_actions_card.png') 0 0 0 0 stretch stretch; border-radius: 12px; }}
        """)
        col = QVBoxLayout(frame)
        col.setContentsMargins(15, 15, 15, 15)
        col.setSpacing(4)
